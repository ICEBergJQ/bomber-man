...
const startingPositions = [
  { row: 1, col: 1 }, { row: 1, col: 21 },
  { row: 11, col: 1 }, { row: 11, col: 21 }
];

let gameState = {
  players: {}, bombs: [], explosions: [], maze: null,
  gameStarted: false, gameOver: false, winner: null, playerCount: 0
};
...
function initializeGame() {
  gameState = {
    ...
    bombs: [], explosions: [], gameStarted: false,
...
function checkWinCondition() {
  ...
  if (alive.length <= 1) {
    ...
    setTimeout(() => { initializeGame(); broadcastGameState(); }, 3000);
  }
}
...
function explodeBomb(bomb) {
  ...
  explosion.forEach(e=>gameState.explosions.push({...e,createdAt:Date.now()}));
  checkPlayerDeaths();
  setTimeout(()=>{ explosion.forEach(e=>gameState.explosions=
    gameState.explosions.filter(ex=>ex.row!==e.row||ex.col!==e.col));
    broadcastGameState();
  },500);
  broadcastGameState();
}
...
function checkPlayerDeaths() {
  Object.values(gameState.players).forEach(p=>{
    if (p.alive && gameState.explosions.some(e=>e.row===p.row&&e.col===p.col)){
      p.alive=false; checkWinCondition();
    }
  });
}
...
function movePlayer(playerId, dir) {
  ...
  if (gameState.maze[nr]?.[nc] && !['#','*'].includes(gameState.maze[nr][nc])) {
    p.row=nr; p.col=nc; broadcastGameState();
  }
}
    ...
    try {
      ...
      if (data.type === 'register' && data.nickname) {
        if (!gameState.players[playerId]) {
          ...
          console.log(`Player ${playerId} registered with nickname "${data.nickname}" at position (${startingPositions[idx].row}, ${startingPositions[idx].col})`);
...
console.log(`Server running on ws://localhost:8080 and ws://${getServerIP()}:8080`);
